...
def jugada(posiciones, jugador):
    casilla = int(input(f"¿Dónde quieres poner la {jugador}? Elija un número entre 1 y 9. "))
...
def victoria(posiciones):
    ganador = 0
    i = 1
    while ganador == 0 and i < 8:
        if posiciones[i] == posiciones[i+1] and posiciones[i] == posiciones[i+2] and posiciones[i] != " ":
            ganador = posiciones[i]
        i+=3

    i = 1
    while ganador == 0 and i < 4:
        if posiciones[i] == posiciones[i+3] and posiciones[i] == posiciones[i+6] and posiciones[i] != " ":
            ganador = posiciones[i]
        i+=1

    i = 1
    while ganador == 0 and i < 4:
        if i == 1 and posiciones[i] == posiciones[i+4] and posiciones[i] == posiciones[i+8] and posiciones[i] != " ":
            ganador = posiciones[i]
        elif i == 3 and posiciones[i] == posiciones[i+2] and posiciones[i] == posiciones[i+4] and posiciones[i] != " ":
            ganador = posiciones[i]
        i+=2
    return ganador
